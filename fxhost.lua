...
local poll_l_in = poll.set("in_l")
local poll_r_in = poll.set("in_r")
...
function init()
  ...
  metro_redraw.event = function()
    ...
    -- Calculate output levels as smoothed version of input
    vu_out_l = vu_out_l * 0.8 + vu_in_l * 0.2
    vu_out_r = vu_out_r * 0.8 + vu_in_r * 0.2
    redraw()
  end
  metro_redraw:start()
...
poll_l_in.callback = function(val)
  vu_in_l = util.clamp(val * 2, 0, 1)
end

poll_r_in.callback = function(val)
  vu_in_r = util.clamp(val * 2, 0, 1)
end
